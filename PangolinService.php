...
class PangolinService
{
    ...
    public function createResource(string $domain, string $targetIp, int $targetPort = 80): string
    {
        ...
        if ($this->siteId) {
            $resourceData['siteId'] = (int) $this->siteId;
        }

        Log::info("[DomainProvisioner/Pangolin] Erstelle Resource: " . json_encode($resourceData));

        // Integration API Pfad: /v1/ (nicht /api/v1/)
        $response = Http::withHeaders($this->headers())
            ->post("{$this->baseUrl}/v1/org/{$this->orgId}/resources", $resourceData);
        ...
        $body       = $response->json();
        $resourceId = $body['data']['resourceId'] ?? $body['data']['id'] ?? $body['resourceId'] ?? null;
        ...
        $targetResponse = Http::withHeaders($this->headers())
            ->post("{$this->baseUrl}/v1/org/{$this->orgId}/resources/{$resourceId}/targets", [
                'ip'      => $targetIp,
                'port'    => $targetPort,
                'method'  => 'http',
                'enabled' => true,
    ...
    {
        $response = Http::withHeaders($this->headers())
            ->delete("{$this->baseUrl}/v1/org/{$this->orgId}/resources/{$resourceId}");
